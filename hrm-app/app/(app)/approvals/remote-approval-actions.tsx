...
export function RemoteApprovalActions({ requestId }: { requestId: string }) {
  ...
  function approve() {
    startTransition(async () => {
      const res = await fetch('/api/remote-work/approve', {
        method: 'POST',
  ...
  function reject() {
    ...
    startTransition(async () => {
      const res = await fetch('/api/remote-work/reject', {
        method: 'POST',
        headers: { 'content-type': 'application/json' },
    ...
    <div className="flex gap-2 self-start md:self-auto">
      ...
      <ResponsiveDialog open={open} onOpenChange={setOpen}>
        ...
        <ResponsiveDialogContent className="sm:max-w-md">
          <ResponsiveDialogHeader>
            <ResponsiveDialogTitle>재택 반려 사유</ResponsiveDialogTitle>
          </ResponsiveDialogHeader>
          <Textarea value={reason} onChange={(e) => setReason(e.target.value)} rows={3} maxLength={500} placeholder="반려 사유" />
